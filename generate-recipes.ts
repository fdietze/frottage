...
async function main() {
  const pictureDescriptions = await Promise.all(dishes.map(async (dish) => {
    ...
    fs.writeFileSync(
      `recipe/${toSnakeCase(dish)}.html`,
      renderHtmlRecipe(dish, dishes, recipe),
    );
    if (dish === dishes[0]) {
      fs.writeFileSync(
        `recipe/index.html`,
...
async function generateName(dish: string) {
  const messages: Array<ChatCompletionMessageParam> = [{
    role: "user",
    content:
      `Invent a short, artful name for a ${dish} recipe that is itself an alliteration, but does not include any of the ingredients.Explain first, in the last line write the name.Examples: Alpine Antipasti, Tangy Teriyaki Tosses, Wild White Whirl, Crispy Coconut Crackers, Fiery Fruit Flirt.`,
  }];
...
async function generateIngredients(name: string) {
  const messages: Array<ChatCompletionMessageParam> = [{
    ...
    role: "user",
    content:
      `List 5 - 10 buyable, preferably fresh ingredients including spices for this recipe.Do not list quantities.No numbers.`,
  }];
  const completion = await openai.chat.completions.create({
    messages: messages,
    model: "gpt-4",
...
async function generatePictureDescription(recipe: string) {
  const messages: Array<ChatCompletionMessageParam> = [{
    role: "user",
    content:
      `Describe concisely and in a few words what can be seen in a photo of the finished recipe: ${recipe} `,
  }];
  const completion = await openai.chat.completions.create({
    messages: messages,
    model: "gpt-4",
...
async function generatePictures(
  prompts: Array<{ dish: string; description: string }>,
): Promise<Array<{ dish: string; url: string }>> {
